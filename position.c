...
/*   Updated: 2018/12/13 12:14:07 by frivaton         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					int *ref_lin)
{
	...
	while (i < 4 && !i_touch)
	{
		...
		while (j < 4 && !i_touch)
		{
			if (g_tetris->tab[i][j] && ((*ref_lin + i) >= g_x_max ||
						(*ref_col + j) >= g_y_max))
				i_touch = 2;
					...
					int *ref_lin)
{
	...
	i_touch = i_touch_tetris(g_tetris, ref_col, ref_lin);
	if (!i_touch)
	{
		ret = i_clone_matrice(g_tetris, *ref_col, *ref_lin);
		return (ret);
	}
	if (*ref_col < g_x_max - 1)
		*ref_col = *ref_col + 1;
	else if (*ref_lin < g_y_max - 1)
	{
		*ref_col = 0;
		*ref_lin = *ref_lin + 1;
	}
		...
		return (0);
	return (i_check_tetris(g_tetris, ref_col, ref_lin));
}
